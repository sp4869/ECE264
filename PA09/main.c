...
int main(int argc, char ** argv)
{
	...
	int ret = List_writeToTextFile(head, "textOutput"); //Should be the same as textOutput_test
	...
	if(ret != EXIT_SUCCESS)
	{
		...
		int d = List_writeToBinaryFile(head, "binaryOutput"); //Should be the same as binaryOutput_test
                List_print(head);              
                List_destroy(head);
		head  = List_readFromBinaryFile("binaryOutput");
                List_print(head);
	if(d != EXIT_SUCCESS)
	{
